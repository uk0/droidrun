...
def filter_a11y_tree_to_interactive_elements(
    ...
    def traverse(node: Dict[str, Any], parent_element: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        results = []
...
def _is_in_screen_bounds(bounds: Dict[str, int], screen_width: int, screen_height: int) -> bool:
...
def _is_interactive_element(node: Dict[str, Any]) -> bool: # possiblity for filtering out non-interactive elements
    return True
