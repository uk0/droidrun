...
class MacroPlayer:
    ...
    async def replay_macro(self, macro_data: Dict[str, Any], 
        ...
        logger.info(f"📊 Success: {success_count}/{total_executed} ({success_rate:.1f}%)")
...
async def replay_macro_file(macro_file_path: str, 
    ...
    """
    player = MacroPlayer(device_serial=device_serial, delay_between_actions=delay_between_actions)
    
    try:
        macro_data = player.load_macro_from_file(macro_file_path)
        return await player.replay_macro(macro_data, start_from_step=start_from_step, max_steps=max_steps)
    except Exception as e:
        logger.error(f"❌ Error replaying macro file {macro_file_path}: {e}")
        return False


async def replay_macro_folder(trajectory_folder: str,
                             device_serial: str = None,
                             delay_between_actions: float = 1.0,
                             start_from_step: int = 0,
                             max_steps: Optional[int] = None) -> bool:
