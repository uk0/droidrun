...
Credentials are never logged or exposed - agents use `type_secret()` to securely input sensitive data. You can also store credentials in a `config/credentials.yaml` file. **→ See [Credential Management Documentation](/features/credentials)** for YAML configuration and advanced usage.
