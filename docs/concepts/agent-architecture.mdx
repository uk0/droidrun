...
description: 'Droidrun hierarchical agent system with specialized roles for planning, execution, and computation.'
---

## What is Multi-Agent Architecture?

Droidrun uses a **hierarchical multi-agent system** where specialized agents work together:
...
**Workflow Steps:**
1. **prepare_context()** → `ManagerContextEvent` - Builds context and system prompt
2. **get_response()** → `ManagerResponseEvent` - Gets LLM response
3. **process_response()** → `ManagerPlanDetailsEvent` - Parses response into plan
4. **finalize()** → `StopEvent` - Returns results to parent workflow
...
**Workflow Steps:**
1. **prepare_context()** → `ExecutorContextEvent` - Builds context and system prompt
2. **get_response()** → `ExecutorResponseEvent` - Gets LLM response
3. **process_response()** → `ExecutorActionEvent` - Parses response into action
4. **execute()** → `ExecutorActionResultEvent` - Executes action on device
5. **finalize()** → `StopEvent` - Returns results to parent workflow
...
ManagerInputEvent → run_manager()
  ↓
  [ManagerAgent Workflow]
  StartEvent → prepare_context()
    ↓
  ManagerContextEvent → get_response()
    ↓
  ManagerResponseEvent → process_response()
    ↓
  ManagerPlanDetailsEvent → finalize()
    ↓
  StopEvent
  ↓
...
ExecutorInputEvent → run_executor()
  ↓
  [ExecutorAgent Workflow]
  StartEvent → prepare_context()
    ↓
  ExecutorContextEvent → get_response()
    ↓
  ExecutorResponseEvent → process_response()
    ↓
  ExecutorActionEvent → execute()
    ↓
  ExecutorActionResultEvent → finalize()
    ↓
  StopEvent
  ↓
