...
```python
from droidrun import DroidAgent, AdbTools, DroidRunConfig
...
# Option 2: Load configuration from YAML file
config = DroidRunConfig.from_yaml("config.yaml")
agent = DroidAgent(
    goal="Find the weather forecast and create a calendar event",
    llm=llm,
    tools=tools,
    agent_config=config.agent,  # Agent-specific configuration
    reasoning=True,
    max_steps=20,
    timeout=600
)
...
**1. Direct Mode (reasoning=False)**
- Uses **CodeActAgent** for immediate execution
- Best for simple, atomic tasks (1-15 steps)
- Generates and executes Python code using device actions
...
agent = DroidAgent(
    ...
    llm=llm,
    tools=tools,
    reasoning=False  # Direct execution
)
```
...
agent = DroidAgent(
    ...
    llm=llm,
    tools=tools,
    reasoning=True  # Manager → Executor workflow
)
```
...
agent = DroidAgent(
    ...
    llms=llms,  # Per-agent LLM configuration
    tools=tools,
    reasoning=True
)
```
...
self.manager_agent = ManagerAgent(
    ...
    shared_state=self.shared_state,  # Shared state
    agent_config=self.config.agent,  # Access from DroidRunConfig instance
    custom_tools=custom_tools
)

# Child agents read and update
...
agent:
  max_steps: 20
  reasoning: true
  after_sleep_action: 0.5  # Seconds to wait after actions

  manager:
    vision: true
    system_prompt: "config/prompts/manager_system.j2"

  executor:
    vision: true
    system_prompt: "config/prompts/executor_system.j2"

  codeact:
    vision: false
    safe_execution: true
    system_prompt: "config/prompts/codeact_system.j2"
  ...
  scripter:
    ...
    execution_timeout: 30
    safe_execution: true
    system_prompt: "config/prompts/scripter_system.j2"
```

### Custom Prompts

Prompts are Jinja2 templates in `droidrun/config/prompts/`:

```jinja2
{# manager_system.j2 #}
You are the Manager agent for device automation.

Your goal is to achieve: {{ instruction }}
...
<current_subgoal>The specific next action</current_subgoal>
```

### Custom Variables

Pass custom variables to agents:
...
# Variables accessible in prompts via {{ variables.departure_city }}
# And in shared_state.custom_variables
```
...
```python
# Simple task
agent = DroidAgent(goal="Open Settings", llm=llm, tools=tools, max_steps=5)

# Complex task
agent = DroidAgent(goal="Multi-app workflow", llm=llm, tools=tools, max_steps=50)
```
...
Trajectories saved to `trajectory_{timestamp}/` directory.
...
class CustomWorkflow(Workflow):
    def __init__(self, tools, manager_llm, codeact_llm, config=None, **kwargs):
...
Solution: Check max_steps and error escalation
```python
# Increase max_steps if task is complex
agent = DroidAgent(goal="...", llm=llm, tools=tools, max_steps=50)

# Check error logs for repeated failures
# Adjust err_to_manager_thresh if needed
```

**Issue: Manager always escalates errors**

Solution: Reduce error threshold or improve prompts
```yaml
agent:
  manager:
    system_prompt: "path/to/custom_prompt.j2"  # Improve error handling
```

**Issue: CodeActAgent generates invalid code**

Solution: Enable safe execution and improve prompt
```yaml
safe_execution:
  codeact:
    safe_execution: true
    allowed_modules: [re, json, datetime]  # Restrict imports
```

**Issue: ScripterAgent never completes**

Solution: Check max_steps and completion signal
```yaml
agent:
  scripter:
...
Solution: Check current text extraction
```python
# Verify focused element has text
state = tools.get_state()
focused = [node for node in state["a11y_tree"] if node.get("focused")]
