...
1. **Coordination Events** - Lightweight routing events for workflow orchestration
2. **Internal Events** - Rich debugging events with full metadata for streaming/logging

```python
# Coordination Event (workflow routing only)
...
This separation ensures efficient workflow coordination while providing rich observability for monitoring and debugging.
...
| `CodeActExecuteEvent` | Trigger CodeAct execution | `task` |
| `CodeActResultEvent` | CodeAct → DroidAgent routing | `success`, `reason`, `task` |
| `ScripterExecutorInputEvent` | Trigger Scripter workflow | `task` |
...
These events are written to the event stream and contain full debugging metadata:
...
DroidAgent maintains shared state accessible across all agents:
...
state = DroidAgentState(
    ...
    # Device state
    formatted_device_state="...",  # XML UI hierarchy
    a11y_tree=[...],              # Raw accessibility tree
    phone_state={...},            # Phone metadata
    screenshot=bytes,             # Current screenshot

    # Action tracking
    action_history=[...],         # All actions taken
    summary_history=[...],        # Action summaries
    action_outcomes=[...],        # Success/failure bools
    error_descriptions=[...],     # Error messages
    ...
    # Script execution
    scripter_history=[...],       # Script results
...
graph LR
    ...
    Final --> Stop[StopEvent]
```

**Events emitted:**
1. `StartEvent` → DroidAgent starts
...
9. `FinalizeEvent` → DroidAgent finalizing
10. Optional: If `output_model` is provided, StructuredOutputAgent extracts structured data
11. `StopEvent` → Workflow complete
...
5. `ScripterThinkingEvent` → Scripter thinking
6. `ScripterExecutionEvent` → Code executing
7. `ScripterExecutionResultEvent` → Execution result
8. `ScripterEndEvent` → Scripter complete
9. Loop back to Manager (step 1)

**Completion:**
- `FinalizeEvent` → Task complete (when Manager provides answer)
- Optional: If `output_model` is provided, StructuredOutputAgent runs to extract structured data from the final answer (uses `StartEvent`/`StopEvent` internally, but these are filtered)
- `StopEvent` → Workflow complete

## Nested Event Handling

DroidAgent streams nested workflow events to its parent context:
...
def handle_stream_event(self, ev: Event, ctx: Context):
    ...
    # Skip StopEvent (workflow-internal)
...
This enables:
- Transparent event bubbling from child to parent workflows
- Trajectory recording at the top level
- Selective event filtering (e.g., episodic memory)
...
async def agent_stream(websocket: WebSocket):
    ...
    try:
        async for event in handler.stream_events():
            event_data = {
                "type": event.__class__.__name__,
                "data": event.dict() if hasattr(event, 'dict') else {},
            }
...
async def automate(ctx, *, goal: str):
    ...
    if result['success']:
        await status_msg.edit(content=f"✅ Success: {result['reason']}")
    else:
        await status_msg.edit(content=f"❌ Failed: {result['reason']}")
```
