...
<Steps>
  ...
  <Step title="Communication Layer">
    DroidRun communicates with the Portal app using two methods:
...
|------------|---------|----------|
| Accessibility Service | Read UI state and perform actions | ✅ Yes |
| Display over other apps | Show element overlay (optional) | ❌ No |
| Query all packages | List installed apps | ✅ Yes |
...
2. Install it on your connected device (with `-g` flag to grant all permissions)
3. Attempt to enable the accessibility service automatically
...
<Tabs>
  ...
  <Tab title="Troubleshooting Installation">
    **Installation fails with "INSTALL_FAILED_UPDATE_INCOMPATIBLE":**
    - Uninstall the existing Portal app first
    - The setup command uses `--uninstall` flag by default
...
<Steps>
  <Step title="Automatic Enablement">
    The `droidrun setup` command attempts automatic enablement via ADB:
    ...
    <Note>
    Automatic enablement may fail on some devices due to security restrictions. If this happens, follow the manual steps below.
    </Note>
  </Step>
  ...
  <Step title="Manual Enablement">
    ...
    2. Navigate to **Accessibility** (path varies by device):
       - Stock Android: Settings > Accessibility
       - Samsung: Settings > Accessibility
       - Xiaomi: Settings > Additional settings > Accessibility
    3. Find **DroidRun Portal** in the services list
    4. Toggle the switch to **ON**
    5. Tap **Allow** on the permission dialog

    The `droidrun setup` command will automatically open the accessibility settings for you.
  </Step>
...
# Test Portal connection (content provider mode)
droidrun ping
...
**Advantages:**
- ⚡ **Faster**: Direct HTTP communication (~2-5x faster)
- 📦 **Efficient**: Binary data transfer for screenshots
- 🔄 **Reliable**: Automatic port reuse, no cleanup needed
...
- Portal listens on device port `8080`
- ADB automatically allocates a free local port (e.g., `37815`)
- Existing forwards are reused (no duplicates)
- Forwards persist until device disconnect
...
2. DroidRun sends ADB shell commands like `content query --uri content://com.droidrun.portal/state`
3. Portal returns data as JSON in the shell output

**Advantages:**
- 🔒 **Always Available**: No network/port requirements
- 🛡️ **Secure**: Uses Android's built-in IPC mechanism
- 🔧 **Reliable**: Works even if TCP server fails

**Limitations:**
- 🐌 **Slower**: ADB shell overhead (~2-5x slower than TCP)
- 📏 **Data size limits**: Shell output may be truncated for large responses

**Automatic fallback:**

DroidRun automatically falls back to content provider mode when:
...
droidrun ping --no-tcp

# Force content provider mode
...
<AccordionGroup>
  ...
  <Accordion title="ContentProvider error: Unknown URI">
    Available endpoints:
    - `/state` - Get UI state and phone state
    - `/keyboard/input` - Input text
    - `/packages` - List installed packages
    - `/overlay_offset` - Set overlay offset

    Ensure you're using the latest Portal app version.
  </Accordion>
</AccordionGroup>
...
tools = IOSTools(
    ...
    bundle_identifiers=["com.example.app"]  # Optional: filter apps
)
...
- ⚠️ Requires manual Portal app installation
  ...
  <Accordion title="Text input not working">
    ...
    **Solutions:**
    ...
    3. **Focus element before input:**
       ...
       tools.tap_by_index(INDEX_OF_TEXT_FIELD)
       time.sleep(0.5)
...
4. **Monitor device battery during long sessions:**
