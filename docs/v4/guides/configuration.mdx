...
description: 'Complete guide to DroidRun configuration including LLM profiles, safe execution, and CLI overrides'
---

# Configuration System

DroidRun uses a powerful YAML-based configuration system that controls every aspect of agent behavior, LLM selection, device settings, and execution safety. This guide covers the complete configuration schema, best practices, and advanced patterns.

<Note>
**For Developers:** Use `DroidRunConfig` class for all configuration operations in your code. `ConfigManager` is for internal CLI use only and should not be used in developer applications.
</Note>
...
2. **Code parameters** (medium priority) - Set when creating DroidAgent
3. **Config file** (lowest priority) - Default values from `config.yaml`

### Configuration File Location

DroidRun searches for `config.yaml` in this order:

1. **Explicit path**: Via `--config` CLI flag or `DroidRunConfig.load(path="/path/to/config.yaml")`
...
If no config file exists, DroidRun automatically creates a default `config.yaml` with sensible defaults.
...
config = DroidRunConfig.load("config.yaml")
...
DroidAgent automatically loads LLMs from config:
...
# LLMs are loaded automatically based on config.llm_profiles
agent = DroidAgent(
    goal="Open settings",
    config=config,  # Uses profiles from config
)
```

### Manual Loading

Load specific profiles programmatically:
...
<Warning>
CLI LLM overrides (`--provider`, `--model`, `--temperature`) apply to ALL agent profiles. For fine-grained control, edit `config.yaml`.
</Warning>
...
**Note:** The CLI override pattern shown here uses `ConfigManager`, which is for **internal CLI use only**. Developers should use `DroidRunConfig` directly (see examples above).

CLI overrides work via **direct mutation**:

```python
# This is for CLI/internal use only - developers use DroidRunConfig
from droidrun.config_manager import ConfigManager

# Load config (singleton for CLI)
config = ConfigManager()

# CLI overrides mutate config directly
...
This ensures CLI flags take precedence without modifying the config file.

**For developers:** Use `DroidRunConfig` to load and modify configuration:
...
def validate_config(config):
    ...
    if config.agent.reasoning and not config.agent.manager.vision:
        raise ValueError("Reasoning mode works best with manager vision enabled")
...
2. Or create manually: `DroidRunConfig().save("config.yaml")`
3. Or specify path: `droidrun run --config /path/to/config.yaml`
