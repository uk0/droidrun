...
DroidRun's structured output system allows you to extract structured, typed data from device automation tasks. By defining a Pydantic model, you can guide the agent to collect specific information and receive it in a predictable format after task completion.

## Overview

Structured output extraction works in two phases:

1. **Task Execution**: The agent performs device actions to complete your goal, collecting the required information along the way
2. **Post-Completion Extraction**: After task completion, a specialized extraction agent analyzes the agent's final answer and extracts structured data based on your Pydantic model

This approach ensures:
- Type-safe data extraction with validation
- Predictable output format for downstream processing
- Separation of concerns between task execution and data extraction
- No performance impact during device interaction
...
async def main():
    ...
    agent = DroidAgent(
        goal="Find John Smith's contact information and tell me his phone number and email",
        config=config,
        tools=AdbTools(),
...
- Agent collects information based on your goal
- Agent's system prompt includes your Pydantic model schema as guidance
...
- Uses LLM's `structured_predict()` to extract data into your Pydantic model
...
By default, structured output extraction uses the `codeact` LLM. You can specify a dedicated `structured_output` LLM profile:
...
The default extraction prompt is simple:
...
For custom prompts, use `StructuredOutputAgent` directly:
...
custom_prompt = PromptTemplate(
    ...
    "Focus on the primary contact details only. "
    "Text:\n\n{text}"
)

# Run extraction with custom prompt
extraction_agent = StructuredOutputAgent(
    llm=llm,
    pydantic_model=ContactInfo,
    answer_text=agent_result["reason"],
)

# Modify prompt (requires accessing internal code)
# extraction_agent.prompt = custom_prompt
